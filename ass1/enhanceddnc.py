...
def closestPair(points, pointsy):	
	distances = []
	left, right = [], []
	distLeft, distRight = [], []
	...
	if len(points) <= 3:
		...
		yLeft.append(ypoints[:sepLine])
		yRight.append(ypoints[sepLine:])
		
		distLeft = closestPair(points[:sepLine], yLeft)

		distRight = closestPair(points[sepLine:], yRight)
		...
		for i in range(len(pointsy)):
			if	pointsy[i][0] >= sepLine - delta and pointsy[i][0] <= sepLine + delta:
				masterY.append(pointsy[i])
		min_pair = crossPoints(masterY, delta)
		
		return min_pair

start = time.time()	
closest_pairs = closestPair(xpoints, ypoints)	
...
output = open("output_enhanceddnc.txt", "w")
